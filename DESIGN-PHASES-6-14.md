...
> **قانون اینستاگرام (تصحیح‌شده):** Graph API استفاده نمی‌شود چون به‌عنوان سرویس‌دهنده از ایران
> در دسترس نیست — نه ممنوعیت، بلکه قید موقت؛ اگر سرویسی (مثل ChatPlace) آن را مهیا کند،
> استقبال می‌کنیم. فعلاً هر قابلیت «رشد فالوور» از طریق زیرساخت خودمان (ManyChat/ChatPlace
> webhook، قیف کامنت→دایرکت، داده‌های `ig_funnel_hits`) پیاده می‌شود.
...
- **فاز ۱۴ (تکمیل):** ChatPlaceClient + سوئیچ `dm.provider`؛ ManyChat غیرفعال به‌عنوان
  fallback.
