...
· **هوشا:** ۰.۹.۶ (`hoosha/`، بخش ۷.۵ در `PROJECT-STATE.md`)
...
محصول IGBZ از nopCommerce (چهار افزونهٔ جدا) به **یک افزونهٔ وردپرس با پنج ماژول
قابل‌خاموش‌وروشن** پورت شده است. شناسهٔ ماژول‌ها در `src/Support/Modules.php` و وضعیت روشن/خاموش
در آپشن `igbz_enabled_modules` ذخیره می‌شود. پیش‌فرض فقط `multitenant` روشن است.
...
ووکامرس با متای `_igbz_tenant_id` محدوده‌بندی می‌شوند (۰/نبود = مشترک پلتفرم).
...
## ۹. صفحات ادمین (۲۸ صفحه — منوی یکپارچهٔ IGBZ)
...
| `hoosha/README.md` | **هوشا** — ابزار عاملِ خودمان (نسخهٔ ۰.۹.۶): نصب، هاب پرووایدر، تونل، پراکسی، لاگ‌ها، ابزارها، MCP، معماری، تست |
