 ...
 *
 * جای فایل‌ها:
 *   ~/.hoosha/agents/<name>.md              سراسری
 *   <workspace>/.hoosha/agents/<name>.md    مخصوص پروژه (اولویت بالاتر)
...
export async function collectAgents( { home, workspace, pluginDirs = [] } ) {
	...
	all.push( ...( await loadAgentsFrom( path.join( workspace, '.hoosha', 'agents' ), 'project' ) ) );
...
export async function saveAgent( roots, def ) {
	...
	const base = def.scope === 'project' ? path.join( roots.workspace, '.hoosha', 'agents' ) : path.join( roots.home, 'agents' );
...
export async function removeAgent( roots, name ) {
	...
	let removed = false;
	for ( const base of [ path.join( roots.home, 'agents' ), path.join( roots.workspace, '.hoosha', 'agents' ) ] ) {
