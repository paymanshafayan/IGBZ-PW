/**
 * API برنامه‌نویسی هوشا — برای وقتی که کسی می‌خواهد هوشا را **داخل** برنامهٔ خودش اجرا کند،
 * نه از راه رابط کاربری.
 *
 * دو سطح:
 *
 *   query()      یک پرسش، یک جواب. ساده‌ترین حالت (همان چیزی که حالت headless می‌سازد).
 *   createHoosha() یک نشست زنده با کنترل کامل: رویدادها، مجوز، ابزار سفارشی.
 *
 * نمونه:
 *
 *   import { query } from 'hoosha';
 *   const out = await query( { prompt: 'تست‌ها را اجرا کن', workspace: '.', mode: 'auto' } );
 *   console.log( out.text );
 *
 *   import { createHoosha } from 'hoosha';
...
/**
 * یک نشست زنده.
 * @param {HooshaOptions} [options]
 */
export async function createHoosha( options = {} ) {
 ...
 *
 * @param {HooshaOptions & {prompt:string, images?:any[]}} options
 * @returns {Promise<{text:string, usage:any, messages:any[], events:any[]}>}
 */
export async function query( options ) {
	/** @type {any[]} */
	const events = [];
	const h = await createHoosha( { ...options, onEvent: ( e ) => events.push( e ) } );
