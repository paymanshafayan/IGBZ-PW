...
export async function diagnose( { home, workspace, config, runtime, providerInfo } ) {
	...
	checks.push( {
		...
		ok: homeWritable,
		detail: home,
		hint: homeWritable ? undefined : 'اجازهٔ نوشتن نیست؛ با HOOSHA_HOME مسیر دیگری بده.',
	} );
	...
	checks.push( {
		name: 'حافظهٔ پروژه',
		ok: true,
		detail: runtime?.projectMemory ? 'HOOSHA.md خوانده شد' : 'HOOSHA.md ندارد',
		hint: runtime?.projectMemory ? undefined : 'با /init یک HOOSHA.md بساز تا عامل قواعد پروژه را بداند.',
	} );
