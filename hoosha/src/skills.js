 ...
 *
 * جای اسکیل‌ها:
 *   ~/.hoosha/skills/<name>/SKILL.md          سراسری (همهٔ پروژه‌ها)
 *   <workspace>/.hoosha/skills/<name>/SKILL.md   مخصوص همین پروژه
 *   ~/.hoosha/plugins/<plugin>/skills/<name>/SKILL.md   از راه پلاگین
 *
...
export async function collectSkills( { home, workspace, pluginDirs = [] } ) {
	...
	all.push( ...( await loadSkillsFrom( path.join( workspace, '.hoosha', 'skills' ), 'project' ) ) );
