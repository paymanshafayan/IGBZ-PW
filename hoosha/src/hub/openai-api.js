...
export function modelsResponse( hub ) {
	const data = Object.values( hub.data.models || {} )
		...
		.map( ( m ) => ( {
			...
			object: 'model',
			created: 0,
			owned_by: hub.data.connections?.[ m.connectionId ]?.label || 'hoosha',
			context_window: m.context || null,
			hoosha: { tags: m.tags, caps: m.caps },
		} ) );

	// نام‌های مجازی هم برگردانده می‌شوند تا مصرف‌کننده بتواند بدون دانستن مدل‌ها بنویسد
	// `model: "auto"` و مسیریابی را به هاب بسپارد.
	data.unshift( { id: 'auto', object: 'model', created: 0, owned_by: 'hoosha', hoosha: { virtual: true } } );
	for ( const combo of Object.values( hub.data.combos || {} ) ) {
		data.push( { id: `combo:${ combo.id }`, object: 'model', created: 0, owned_by: 'hoosha', hoosha: { virtual: true, label: combo.label } } );
	}
