/**
 * کلاینت MCP — اتصال به سرورهای بیرونی و آوردن ابزارهایشان داخل هوشا.
 *
 ...
 *   ~/.hoosha/config.json  → کلید mcpServers   (سراسری)
 *   <workspace>/.hoosha/mcp.json               (مخصوص پروژه)
 *
...
export class McpManager {
	...
	async connectAll( { home, workspace, servers = {} } ) {
		...
		try {
			const raw = await fs.readFile( path.join( workspace, '.hoosha', 'mcp.json' ), 'utf8' );
	...
	async #connectOne( name, cfg, home ) {
		...
		const client = new Client( { name: 'hoosha', version: '0.2.0' }, { capabilities: {} } );
