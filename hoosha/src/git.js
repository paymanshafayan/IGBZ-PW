 ...
 *
 * خواستهٔ کارفرما (سند `DESIGN-HUSHVARE.md` §۱۶): مدیری که وسط خرابی نشسته باید بدون هیچ
...
/** شاخه‌هایی که هوشا مستقیم رویشان کار نمی‌کند. */
...
export async function commit( dir, { message, paths, branch: wanted } ) {
	...
	if ( before.protected || wanted ) {
		const name = wanted || `hoosha/${ slug( text ) }-${ Date.now().toString( 36 ).slice( -4 ) }`;
	...
	const out = await git(
		[ '-c', 'user.name=Hoosha', '-c', 'user.email=hoosha@igbz.local', 'commit', '-m', text ],
		{ cwd: dir }
	);
	if ( ! out.ok ) {
...
export async function push( dir, { branch: name, token, username = 'x-access-token' } = {} ) {
	...
	if ( token ) {
		helper = await fs.mkdtemp( path.join( os.tmpdir(), 'hoosha-cred-' ) );
		...
		await fs.writeFile(
			...
			[
				'#!/bin/sh',
				'case "$1" in',
				`*[Uu]sername*) printf '%s' "$HOOSHA_GIT_USER" ;;`,
				`*[Pp]assword*) printf '%s' "$HOOSHA_GIT_TOKEN" ;;`,
				'esac',
				'',
			].join( '\n' ),
			{ mode: 0o700 }
		...
		env = {
			...
			HOOSHA_GIT_USER: username,
			HOOSHA_GIT_TOKEN: token,
		};
	}
...
export async function clone( { url, into, name, token, username = 'x-access-token', branch: wanted } ) {
	...
	if ( token ) {
		...
		await fs.writeFile(
			file,
			[ '#!/bin/sh', 'case "$1" in', `*[Uu]sername*) printf '%s' "$HOOSHA_GIT_USER" ;;`, `*[Pp]assword*) printf '%s' "$HOOSHA_GIT_TOKEN" ;;`, 'esac', '' ].join( '\n' ),
			{ mode: 0o700 }
		);
		env = { ...process.env, GIT_ASKPASS: file, GIT_TERMINAL_PROMPT: '0', HOOSHA_GIT_USER: username, HOOSHA_GIT_TOKEN: token };
	}
