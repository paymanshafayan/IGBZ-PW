 ...
 * `~/.hoosha/tunnel/core/`. بخشی از تونل هوشا (۰.۹.۶).
 */
...
export async function downloadCore( onProgress = () => {} ) {
	...
	try {
		...
		const rel = await fetch( `https://api.github.com/repos/${ REPO }/releases/latest`, {
			headers: { 'user-agent': 'hoosha-tunnel' }, signal: AbortSignal.timeout( 20_000 ),
		} ).then( ( r ) => r.json() );
		...
		const zipBuf = Buffer.from( await ( await fetch( asset.browser_download_url, {
			headers: { 'user-agent': 'hoosha-tunnel' }, signal: AbortSignal.timeout( 300_000 ),
		} ) ).arrayBuffer() );
