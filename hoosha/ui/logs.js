...
export async function renderLogsSettings( box ) {
	...
	const exportBtn = h( 'button', {
		...
		onClick: async () => {
			...
			const a = h( 'a', { href: URL.createObjectURL( new Blob( [ blob ], { type: 'application/json' } ) ), download: 'hoosha-logs.json' } );
	...
	box.appendChild( h( 'div', { class: 'form-card' }, [
		h( 'h4', { text: 'لاگ‌های هوشا' } ),
		h( 'p', { class: 'note', text: 'همهٔ رخدادها و خطاها با کانال و زمان. «زمینه» جزئیات JSON هر ردیف را باز می‌کند. نسخهٔ کامل روی دیسک: logs/hoosha.log در پوشهٔ خانگی هوشا.' } ),
