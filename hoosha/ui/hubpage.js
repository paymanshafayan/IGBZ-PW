...
function statusBar( box ) {
	...
	let note = 'حالت سادهٔ تک‌پروفایدر فعال است؛ هوشا فقط از پروفایل قدیمی استفاده می‌کند.';
	let cta = null;
...
function renderProviderModels( box, page, conns, allModels ) {
	box.appendChild( section( 'مدل‌های این سرویس', 'کشف خودکار نقطهٔ شروع است؛ برچسب و قیمت را خودت درست کن و هوشا از نتیجهٔ واقعی یاد می‌گیرد.' ) );
...
function renderCombos( box, page ) {
	...
	box.appendChild( h( 'div', { class: 'form-card' }, [
		...
		h( 'p', { class: 'note', text: 'یک متن نمونه بنویس و ببین هوشا آن را چه جنسی می‌فهمد و به کدام مدل می‌فرستد — بدون تماس واقعی.' } ),
		probe,
		row(
