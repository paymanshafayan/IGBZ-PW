#!/usr/bin/env bash
# ---------------------------------------------------------------------------
#  بازگرداندن هوشا به آخرین بکاپ سالم.
#
...
archive="$( ls -1t "$here"/hoosha-*.tar.gz 2>/dev/null | head -1 || true )"
if [ -z "$archive" ]; then
...
if [ -d hoosha ]; then
	...
	tar --exclude=node_modules --exclude=.deps-marker -czf "$panic" hoosha hoosha.sh hoosha.cmd 2>/dev/null || true
...
( cd hoosha && npm ci >/dev/null 2>&1 ) || {
	printf '  ✗ npm ci شکست خورد. خودت یک بار «cd hoosha && npm ci» بزن.\n\n' >&2
	exit 1
}

printf '\n  ✓ برگشت به: %s\n' "$( cd hoosha && node -p "require('./package.json').version" )"
printf '    حالا این را بزن تا مطمئن شوی:  cd hoosha && node test/run.mjs\n'
